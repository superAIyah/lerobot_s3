...
class LeRobotDatasetMetadata:
    ...
    def __init__(
        ...
        Args:
            ...
            s3_endpoint_url: Custom S3-compatible endpoint URL (e.g. Sbercloud OBS).
            max_pool_connections: Maximum number of S3 connection pool connections.
        """
        self.repo_id = repo_id
        self.revision = revision if revision else CODEBASE_VERSION

        if str(root or "").startswith("s3://"):
            load_dotenv()
            ...
            self._requested_root = Path(root, **upath_params)
        else:
